...
export class CourseService {
    ...
    async addCourse(CourseData: any): Promise < Course | null > {
        ...
        const CourseDatas = {
            ...
            lessons: CourseData.addCurriculumData,
...
async getAcceptedCourse(){
    try {
        const courses =await this.courseModel.find({isPublished:true,status:"accepted"}).sort({createdAt:-1}).populate('category')
        if(!courses){
...
async findOne(id: string): Promise<Course> {
  ...
  async update(id: string, CourseData: any): Promise<Course> {
    ...
    console.log(CourseDatas,'Course dataas........');
